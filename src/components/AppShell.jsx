...
export default function AppShell({ children, activeTab, setActiveTab }) {
  ...
  useEffect(() => {
    if (!user?.id) return;
    const fetch = () => {
      apiCall(`/api/notifikacije/korisnik/${user.id}/broj-neprocitanih`).then(d => {
        setNotifCount(d?.brojNeprocitanih || 0);
      }).catch(() => {});
    };
    fetch();
    const interval = setInterval(fetch, 30000); // svakih 30s
    return () => clearInterval(interval);
  }, [user?.id]);

  // Resetuj badge kada klikneš na notifikacije
  const handleTab = (id) => {
    if (id === "notifikacije") setNotifCount(0);
    setActiveTab(id);
  };
  ...
  const nav = [
    ...
    { id: "notifikacije",   label: "Obavijesti",      Icon: Icon.Bell, badge: notifCount },
    ...((user?.uloga === "RUKOVODILAC" || user?.uloga === "ADMIN") ? [{ id: "admin", label: "Korisnici", Icon: Icon.Users }] : []),
      ...
      <header style={{
        ...
        <nav style={{ display: "flex", gap: 2 }}>
          {nav.map(({ id, label, Icon: Ic, badge }) => (
            <button key={id} onClick={() => handleTab(id)} className={`nav-btn ${activeTab === id ? "active" : ""}`} style={{ position: "relative" }}>
              <Ic/> {label}
              {badge > 0 && (
                <span style={{
          ...
          <div style={{
            ...
            {user?.uloga && user.uloga !== "GRADJANIN" && (
              <span style={{
                ...
                color: "#F39C12", padding: "1px 5px", borderRadius: 3,
