...
export default function PrijaveListPage() {
    ...
    .filter((p) => {
      ...
      if (search) {
        ...
        if (
          ...
          !p.nazivKategorije?.toLowerCase().includes(q)
        )
          return false;
      }
      return true;
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <PageHeader title="Prijave" sub={`${filtered.length} od ${prijave.length} prijava`} />
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {["Sve", "HITNO", "VISOK", "SREDNJI", "NIZAK"].map((p) => (
              <button
                key={p}
                onClick={() => setFPrio(p)}
                className={`filter-chip ${fPrio === p ? "active" : ""}`}
                style={fPrio === p && p !== "Sve" ? { background: PRIO_CFG[p]?.dim, borderColor: PRIO_CFG[p]?.color + "60", color: PRIO_CFG[p]?.color } : {}}
              >
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 80, color: T.textMuted, fontSize: 13 }}>
          ...
          <button onClick={fetchPrijave} className="btn-ghost" style={{ fontSize: 12 }}>Pokušaj ponovo</button>
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: 80, color: T.textMuted, fontSize: 13 }}>
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: cols, gap: 12, padding: "10px 24px", background: T.bgRaised, borderBottom: `1px solid ${T.line}`, fontSize: 11, fontWeight: 500, color: T.textMuted, letterSpacing: "0.05em", textTransform: "uppercase" }}>
          ...
          {filtered.map((p, i) => (
            ...
            <div
              ...
              style={{ gridTemplateColumns: cols, gap: 12, height: 56, animationDelay: `${Math.min(i * 0.02, 0.4)}s` }}
            >
              <div style={{ overflow: "hidden" }}>
                ...
                {p.adresa && (
                  <div style={{ fontSize: 11, color: T.textMuted, display: "flex", alignItems: "center", gap: 3, marginTop: 2 }}>
                    <Icon.Pin />{p.adresa}
                  </div>
                )}
              </div>
